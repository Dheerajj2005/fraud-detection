    ...
    with col1:
        ...
        with col_time:
            time_val = st.number_input(
                "Time (seconds)",
                min_value=0.0,
                value=float(st.session_state.sample_data["Time"])
        ...
        with col_amount:
            amount_val = st.number_input(
                "Amount ($)",
                min_value=0.0,
                value=float(st.session_state.sample_data["Amount"])
                if st.session_state.sample_data
                else 100.0,
        ...
        with st.expander("🔢 PCA Features (V1-V28)", expanded=False):
            st.markdown(
                "*These are PCA-transformed features from the original dataset*"
            )
            ...
            for i in range(0, 28, 4):
                ...
                for j, col in enumerate(cols):
                    if i + j < 28:
                        ...
                        with col:
                            default_val = (
                                st.session_state.sample_data[f"V{v_num}"]
                                if st.session_state.sample_data
                                else 0.0
                            )
        ...
        if st.button("🎯 Check Transaction", type="primary", use_container_width=True):
            # Prepare transaction
            transaction = {"Time": time_val, "Amount": amount_val, **v_features}
            ...
            with st.spinner("Analyzing transaction..."):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if result["is_fraud"]:
                            st.error(
                                f"🚨 **FRAUD DETECTED** - Risk Level: {result['risk_level']}"
                            )
                        else:
                            st.success(
                                f"✅ **LEGITIMATE TRANSACTION** - Risk Level: {result['risk_level']}"
                            )
                        ...
                        with col3:
                            st.metric(
                                "Response Time", f"{result['response_time_ms']:.2f}ms"
                            )
...
with tab2:
    ...
    st.info(
        "Upload a CSV file with transaction data. File must include Time, Amount, and V1-V28 columns."
    )
                    ...
                    with st.spinner(f"Processing {len(df)} transactions..."):
                        try:
                            ...
                            if response.status_code == 200:
                                ...
                                # Show results
                                st.success(
                                    f"✅ Processed {result['total_transactions']} transactions in {duration:.2f}s"
                                )
                                ...
                                with col1:
                                    st.metric(
                                        "Total Processed", result["total_transactions"]
                                    )
                                ...
                                output_df = pd.concat(
                                    [df[["Time", "Amount"]], results_df], axis=1
                                )
