...
with col2:
    ...
    fraud_series = filtered_df.set_index('timestamp').resample(freq).agg({
        'is_fraud': ['sum', 'count']
    })
    fraud_series.columns = ['fraud_count', 'total']
    fraud_series['fraud_rate'] = fraud_series['fraud_count'] / fraud_series['total'] * 100
...
with col2:
    ...
    fig = go.Figure(data=[go.Bar(
        ...
        marker_color=[colors.get(level, '#95a5a6') for level in risk_counts.index],
...
with col2:
    hourly_fraud = hourly_stats.groupby('hour')['is_fraud'].agg(['sum', 'count']).reset_index()
...
with col1:
    sort_by = st.selectbox(
        "Sort by",
        ['Timestamp (Recent First)', 'Amount (High to Low)', 'Probability (High to Low)', 'Latency (High to Low)']
    )
...
display_df_formatted['timestamp'] = display_df_formatted['timestamp'].dt.strftime('%Y-%m-%d %H:%M:%S')
display_df_formatted['amount'] = display_df_formatted['amount'].apply(lambda x: f"${x:.2f}")
display_df_formatted['fraud_probability'] = display_df_formatted['fraud_probability'].apply(lambda x: f"{x:.2%}")
display_df_formatted['is_fraud'] = display_df_formatted['is_fraud'].apply(lambda x: '🚨 Fraud' if x else '✅ Safe')
display_df_formatted['latency_ms'] = display_df_formatted['latency_ms'].apply(lambda x: f"{x:.2f}ms")

display_df_formatted = display_df_formatted[['timestamp', 'amount', 'fraud_probability', 'is_fraud', 'risk_level', 'latency_ms']]
display_df_formatted.columns = ['Timestamp', 'Amount', 'Fraud Prob.', 'Status', 'Risk', 'Latency']

# Pagination
total_pages = len(display_df_formatted) // page_size + (1 if len(display_df_formatted) % page_size > 0 else 0)
...
if total_pages > 1:
    ...
    st.caption(f"Page {page} of {total_pages} ({len(display_df_formatted)} total records)")
else:
    st.dataframe(
        display_df_formatted.head(page_size),
        use_container_width=True,
        hide_index=True
    )
