...
fpr = metrics.get("false_positive_rate") if "false_positive_rate" in metrics else compute_fpr(metrics)
...
with col2:
    st.metric("Recall (Fraud)", f"{metrics.get('recall', 0):.2%}", help="Percentage of frauds detected")
with col3:
    st.metric("Precision (Fraud)", f"{metrics.get('precision', 0):.2%}", help="Percentage of correct fraud predictions")
with col4:
    st.metric("F1-Score", f"{metrics.get('f1_score', 0):.2%}", help="Harmonic mean of precision and recall")
...
display_image(figures_path / "test_confusion_matrix.png", "Confusion Matrix (Counts)", col_img1)
display_image(figures_path / "test_confusion_matrix_normalized.png", "Confusion Matrix (Normalized)", col_img2)
...
if 'threshold_analysis' in metrics:
    ...
    with st.expander("📋 Detailed Threshold Metrics"):
        st.dataframe(pd.DataFrame(ta['all_thresholds']).style.format("{:.4f}"), use_container_width=True)
else:
...
if 'business_metrics' in metrics:
    ...
    b3.metric("Net Benefit", f"${bm['net_benefit']:,.2f}", delta=f"${bm['net_benefit']:,.2f}")
    
    cost_data = pd.DataFrame({
        'Category': ['TP Benefit', 'FP Cost', 'FN Cost'],
        'Amount': [bm['true_positive_benefit'], -bm['false_positive_cost'], -bm['false_negative_cost']]
    })
